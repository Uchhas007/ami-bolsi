...
from flask_mail import Mail # to send mail using flask from our browser for notification purposes 
...
from werkzeug.utils import secure_filename # for the privacy of the files uploaded in our website 
...
wn = params['websitename'] # storing the name of the website from the params in a new var called wn
...
app.config['UPLOAD_FOLDER'] = params['upload_location'] # storing the uplaod location in this variable
...
def home():
    ...
    posts = Posts.query.filter_by().all() # fetching all the fucking post using this line

    last = math.ceil(len(posts)/int(params['no_of_posts'])) # finding the last page using this line 
    ...
    page_number = (request.args.get('page')) # fetching the page number using this line. And this thing will always be a string 
    # Our web page url is string. So we convert the page number into integer for calculation puposes 

    if (not str(page_number).isnumeric()): # verifying if the page number is numeric value or not
    ...
    posts = posts[(page_num-1)*int(params['no_of_posts']):(page_num-1)*int(params['no_of_posts'])+ int(params['no_of_posts'])]
    ...
    if page_num==1: # if we're on the first page then...
        ...
        next = "/?page="+ str(page_num+1) # the next page will be page + 1. We did so and also type casted the value back to string hence url is always a string 
    
    # if we are on our last page then...
    elif page_num == last: 
        prev = "/?page="+ str(page_num-1) # prev page before the last page is page - 1
...
def dashboard():
    ...
    if "user" in session and session['user']==params['admin_user']: # verifying if admin is already logged in or not
        post = Posts.query.all() # if logged in then admin can see all posts from his dashboard
        return render_template("admin.html", params = params, posts = post) # we'll pass all the posts & params in frontend

    if request.method=="POST": # if admin isn't logged in then he have to sign in
        useremail = request.form.get("email") # fetching the email from the form in login.html. 
        userpass = request.form.get("pass") # fetching the email from the form in login.html.
        # have written name = email and name = pass in the login.html. This is the varible name used in login.html to store our data

        if useremail == params['admin_user'] and userpass == params['admin_pass']: # matching data with username & password stored in json
        ...
        return render_template("login.html", params = params) # we'll send back to the login page once again for privacy
...
def uploader():
    if "user" in session and session['user']==params['admin_user']: # if the user is logged in then he/she can upload the file 
        if request.method=='POST': # the user must request the post instead of typing the url manually in the browser 
            f = request.files['file1'] # will fetch the file from the admin panel which is named as file1 and store it in f var
            f.save(os.path.join(app.config['UPLOAD_FOLDER'], secure_filename(f.filename))) # then we'll save the file here 
            return "Uploaded successfully!"  # returning a success  message bro

@app.route("/delete/<string:sno>" , methods=['GET', 'POST'])
def delete(sno):
    if "user" in session and session['user']==params['admin_user']: # only if the user is logged in then he/she can delete the post
        post = Posts.query.filter_by(sno=sno).first() # fetching the post that we want to delte  
